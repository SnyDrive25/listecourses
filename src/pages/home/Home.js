...
function Home() {
  ...
  function sync() {
    if (localStorage.getItem("liste") !== null) {
      var array = JSON.parse(localStorage.getItem("liste"));
      console.log(array);
    }
    else {
      console.log("nope");
  ...
  function search() {
    var element = document.getElementById("new_element").value.replace(/[^a-zA-Z0-9\s]/g, "");   // On enlève tout ce qui n'est pas un caractère alphabétique (a-zA-Z), numériques (0-9) ou un espace (\s) avec une expression régulière
    if (element.length === 0) {
  ...
  return (
    <div className="App">
      ...
      <button className='sync' onClick={() => sync()}>Synchroniser avec les données du navigateur</button>
      <div className='card'>
